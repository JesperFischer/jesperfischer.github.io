...
#df = read.csv("raw_hrd.csv") %>% filter(participant_id == "sub-0019" & Modality == "Extero")
             ...
             sidebarLayout(
               sidebarPanel(
                 ...
                 column(width = 6,
                        ...
                        sliderInput("conf_prec", "conf_prec", min = -10, max = 10, value = 0, step = 0.1)
                 )
               ),
               mainPanel(
                 plotOutput("results", height = "700px")
               )
             )
    )
  ),
